...
def plot_map(xr,yr,variable,variable_name,title,output_name,vmin,vmax,colorbar):
   ...
   if variable_name=='icet':
      ...
      cbar.ax.set_xlabel('Salt flux (kg.m$^{-2}$.d$^{-1}$)')
...
def plot_map_ano(xr,yr,variable,variable_name,y1,title,output_file,vmin,vmax,colorbar):
   ...
   if variable_name=='ice_f':
      ...
      cbar.ax.set_xlabel('Salt flux anomaly (kg.m$^{-2}$.d$^{-1}$)')
...
def vertical_profile(var1,var2,var3,y1,l1,l2,l3,variable_name,vmin,vmax,z,zmax,ocean,lat_min,name_file):
   ...
   fig,ax = plt.subplots()
   ax.plot(var1[0:i_zmax+1],z[0:i_zmax+1],label=str(l1.replace("_"," ")))
   ax.plot(var2[0:i_zmax+1],z[0:i_zmax+1],label=str(l2.replace("_"," ")))
   ax.plot(var3[0:i_zmax+1],z[0:i_zmax+1],label=str(l3.replace("_"," ")))
...
def vertical_profile_div(var1,var2,y1,y2,variable_name,vmin,vmax,z,zmax,ocean,lat_min,name_file):
   ...
   #ax.plot(var3[0:i_zmax+1],z[0:i_zmax+1],label=str(y3))
   plt.gca().invert_yaxis()
   plt.ylabel(r'Depth (m)')
   plt.ylim([z[i_zmax],0])
   plt.xlim([vmin,vmax])
   ...
   ax2.plot(var1[i_zmax:-4],z[i_zmax:-4])
   ax2.plot(var2[i_zmax:-4],z[i_zmax:-4])
   #ax2.plot(var3[i_zmax:-4],z[i_zmax:-4])
   ...
   if ocean == 'undefined':
      plt.suptitle('Arctic mediterranean seas ($>$'+str(lat_min)+'$^{o}$N)')
   else:
      plt.suptitle(str(ocean.replace("_"," ")))
   '''
   box = ax.get_position()
   ax.set_position([box.x0, box.y0 + box.height * 0.3,
                 box.width, box.height * 0.7])
