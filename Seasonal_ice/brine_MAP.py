...
class From1950to2100():                                                                 #//
  ...
  def __init__(self,option,var,y1,y2,colorbar,basin):                                            #//
     ...
     self.y2f = 2090     
     if var == 'brine':                                                                  #//
        self.path = '/media/fig010/LACIE SHARE/EC_data/yearly'+str(y1)+'-'+str(y2)+'/'\
                  'salt_'+str(y1)+'-'+str(y2)+'.nc'
     elif var == 'ice_f':
        self.path = '/media/fig010/LACIE SHARE/EC_data/yearly'+str(y1)+'-'+str(y2)+'/'\
                  'icef_'+str(y1)+'-'+str(y2)+'.nc'

     if basin != 'undefined':
       self.output_fileC = str(var)+'_'+str(basin)+'_10yAnomaly_'+str(self.y1h)+'-'+str(self.y1f)+'_'+str(colorbar)                                #//
       self.output_fileH = str(var)+'_'+str(basin)+'_'+str(self.y1h)+'-'+str(self.y2h)+'_'+str(colorbar)                                #//
       self.output_fileF = str(var)+'_'+str(basin)+'_'+str(self.y1f)+'-'+str(self.y2f)+'_'+str(colorbar)                                #//
     else:
       self.output_fileC = str(var)+'_10yAnomaly_'+str(self.y1h)+'-'+str(self.y1f)+'_'+str(colorbar)                                #//
       self.output_fileH = str(var)+'_'+str(self.y1h)+'-'+str(self.y2h)+'_'+str(colorbar)                                #//
       self.output_fileF = str(var)+'_'+str(self.y1f)+'-'+str(self.y2f)+'_'+str(colorbar)                                #//
...
array_to_plotC = -np.nanmean(IceExt[:,:,index_y1h:index_y1h+10],axis=2)+np.nanmean(IceExt[:,:,index_y1f:index_y1f+10],axis=2)
...
make_plot.plot_map(xr,yr,array_to_plotH,var,'',simu.output_fileH,vmin,vmax,colorbar)
make_plot.plot_map(xr,yr,array_to_plotF,var,'',simu.output_fileF,vmin,vmax,colorbar)
make_plot.plot_map_ano(xr,yr,array_to_plotC,var,simu.y1f,'',simu.output_fileC,vmin,vmax,colorbar)
