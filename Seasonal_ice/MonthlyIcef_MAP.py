...
class From1950to2100():                                                                 #//
  ...
  def __init__(self,option,var,y1,y2,colorbar,basin):                                            #//
     ...
     #self.path_M = '/media/fig010/LACIE SHARE/EC_data/March'+str(y1)+'-'+str(y2)+'/'\
     #             'icefMarch_1950-2100.nc'
     ...
     if basin != 'undefined':
       self.output_fileC = str(var)+'monthly_'+str(basin)+'_10yAnomaly_'+str(self.y1h)+'-'+str(self.y1f)+'_'+str(colorbar)                                #//
       self.output_fileH = str(var)+'monthly_'+str(basin)+'_'+str(self.y1h)+'-'+str(self.y2h)+'_'+str(colorbar)                                #//
       self.output_fileF = str(var)+'monthly_'+str(basin)+'_'+str(self.y1f)+'-'+str(self.y2f)+'_'+str(colorbar)                                #//
     else:
       self.output_fileC = str(var)+'monthly_10yAnomaly_'+str(self.y1h)+'-'+str(self.y1f)+'_'+str(colorbar)                                #//
       self.output_fileH = str(var)+'monthly_'+str(self.y1h)+'-'+str(self.y2h)+'_'+str(colorbar)                                #//
       self.output_fileF = str(var)+'monthly_'+str(self.y1f)+'-'+str(self.y2f)+'_'+str(colorbar)                                #//
...
#IceFS = loading.extracting_var(simu.path_S, var)
#IceFS[np.where(IceFS>1E10)]= 0
#IceFS[np.where(IceFS<0)] = 0
...
hist = np.zeros((2100-1950))
for i in range(0,2100-1950):
   hist[i] = np.nansum(sub[:,:,i*12:i*12+12])

timebis = np.linspace(1950,150,2100)
time2=np.arange(1950,2100,1)
print(timebis,time2)
plt.figure(1)
plt.plot(time2,hist)
plt.savefig('montlysum_histo'+str(basin)+'.png')

'''
array_to_plotH = np.nansum(IceF[:,:,index_y1h:index_y1h+10*12],axis=2)/10
#array_to_plotH[array_to_plotH==np.nan] = 0

array_to_plotF = np.nansum(IceF[:,:,index_y1f:index_y1f+10*12],axis=2)/10
...
make_plot.plot_map(xr,yr,array_to_plotH*1E2,var,title,simu.output_fileH,vmin,vmax,colorbar)
title= str(simu.y1f)+'-'+str(simu.y2f)
make_plot.plot_map(xr,yr,array_to_plotF*1E2,var,title,simu.output_fileF,vmin,vmax,colorbar)
make_plot.plot_map_ano(xr,yr,array_to_plotC*1E2,var,simu.y1f,'future-current',simu.output_fileC,vmin,vmax,colorbar)
'''
